...
export const AtlasDataTable: React.FunctionComponent<AtlasDataTableProps> = ({
  ...
  if (iframeURL) {
    return (
      <Portal node={document && document.getElementById("iframe-wrapper")}>
        <div className={"text-center"}>
          ...
          >
            <i className="fas fa-arrow-circle-left"></i> Back to list
          </a>
        ...
        <table className={"table table-striped"}>
          <thead>
            <tr>
              {atts.map((att) => (
                <th
                  className={`col_${att.name.replace(" ", "_").toLowerCase()}`}
                  key={att.name}
                >
                    ...
                    {vals.map((val: any, j: number) => {
                      ...
                      const meta =
                        att && att.schemaMetadata
                          ? JSON.stringify(att.schemaMetadata)
                          : JSON.stringify({});
