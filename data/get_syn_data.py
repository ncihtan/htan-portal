...
def generate_json(include_at_risk_populations, include_released_only, do_not_download_from_synapse):
    ...
    if include_released_only:
        released_entities_df = pd.read_csv("entities_v7_0.csv")
    ...
    released_metadata_df = pd.read_csv("metadata_v7_0.csv")
        ...
        for dataset in datasets:
            ...
            if not pd.isna(released_record["Manifest_Version"]):
                try:
                    ...
                    logging.warning(f"Invalid version for {component}/{dataset['id']}: {released_record['Manifest_Version']}")
            try:
